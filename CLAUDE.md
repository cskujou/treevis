...
- **Frontend**: `static/app.js` handles data input, list management, sorting, drag reorder, and rendering
...
- `loadNewTree()` in `static/app.js` renders the tree; `renderEmptyState()` handles no-data state
- `updateFileListUI()` renders the sidebar list and binds remove/drag handlers
- `applySort()` supports list sorting modes (`manual`, `name`, `time`)
- Drag reorder is enabled only when sort mode is `manual`
- `buildTree()` recursively creates DOM elements from parsed tree data
